...
class MT_Dataset(Dataset):
    ...
    @classmethod
    def load_data_and_create_vectorizer(cls, data_dir, first_person_only=False,
                                        add_side_constraints=False):
        raw_dataset = RawDataset(data_dir, first_person_only=first_person_only)
    ...
    def __getitem__(self, index):
        ...
        vectorized = self.vectorizer.vectorize(src_token, trg_token,
                                               trg_gender)
        return vectorized
...
def inference(sampler, beam_sampler, dataloader, args):
    ...
    for batch in dataloader:
        ...
        beam_trans_10 = beam_sampler.beam_decode(sentence=src,
                                                 first_person_only=args.first_person_only,
                                                 add_side_constraints=args.add_side_constraints,
                                                 trg_gender=trg_gender, topk=1, beam_width=15, max_len=512)
...
def get_morph_features(args, data, word_vocab):
    ...
    if args.reload_files:
        ...
        morph_featurizer.featurize(data)
        if args.cache_files:
...
def main():
    ...
    parser.add_argument(
        "--first_person_only",
        action="store_true",
        help="To do first person only reinflection. If False, the model "
             "defaults to multi-user reinfelction"
    )
    parser.add_argument(
        "--vectorizer_path",
        default=None,
    ...
    if args.use_morph_features:
        ...
        train_src_data = [t.src_token for t in dataset.train_examples]
        morph_embeddings = get_morph_features(args, train_src_data,
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            dataloader = DataLoader(dataset, shuffle=True, batch_size=args.batch_size, collate_fn=collator, drop_last=False)
