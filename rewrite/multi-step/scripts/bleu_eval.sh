...
GOLD_DATA_DIR=/home/ba63/gender-rewriting/data/m2_edits/v2.1/norm_data/MT
split=test
...
if [ "$EXPERIMENT" = "google_translate" ]; then
    ...
    printf "Evaluation Against $f:\n"
        cat $DATA_DIR/$split.google.ar.norm | sacrebleu $f -w 2 --force
    done
    ...
    cat $DATA_DIR/arin.to.MM.preds.norm | sacrebleu $GOLD_DATA_DIR/$split.ar.MM.norm -w 2 --force

    printf "Evaluating  $DATA_DIR/arin.to.FM.preds.norm against $GOLD_DATA_DIR/$split.ar.FM.norm:\n"
    cat $DATA_DIR/arin.to.FM.preds.norm | sacrebleu $GOLD_DATA_DIR/$split.ar.FM.norm -w 2 --force

    printf "Evaluating  $DATA_DIR/arin.to.MF.preds.norm against $GOLD_DATA_DIR/$split.ar.MF.norm:\n"
    cat $DATA_DIR/arin.to.MF.preds.norm | sacrebleu $GOLD_DATA_DIR/$split.ar.MF.norm -w 2 --force

    printf "Evaluating  $DATA_DIR/arin.to.FF.preds.norm against $GOLD_DATA_DIR/$split.ar.FF.norm:\n"
    cat $DATA_DIR/arin.to.FF.preds.norm | sacrebleu $GOLD_DATA_DIR/$split.ar.FF.norm -w 2 --force
